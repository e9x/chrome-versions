...
const getRecoveryURL = (image, secure = true) =>
  `http${
    ...
    image.mp_key === 1 ? "mp" : `mp-v${image.mp_key}`
  }.bin.zip`;
...
const parseRecoveryURL = (url) => {
  ...
  const match = binName.match(
    /chromeos_([\d\.]+)_(\w+)_recovery_(stable|beta|dev)-channel_mp(-v[1-9][0-9]*)?\.bin\.zip/
  );
  if (!match) throw new TypeError("Bad recovery image URL");
  const [platform, codename, channel, mp_key] = match;

  return {
    mp_key: mp_key === "" ? 1 : parseInt(mp_key),
    board: codename,
    channel,
    platform,
  };
};
exports.parseRecoveryURL = parseRecoveryURL;
